...
- **Learn loop (partial)** — **recommendation-engagement** capture; **gap-driven open questions** that raise *and retract* themselves as data fills (e.g. "add your product costs"). **(2026-07-30: now also clearance **outcome measurement** → `business.clearance_effectiveness.all_time`, plus `business.action_decline_signal.all_time` from declined suggestions, refreshed by a daily worker job — both dormant until execution is live.)**
