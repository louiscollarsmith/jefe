...
3. **Now: the Kinetic layer.** The marginal *belief* has diminishing returns — depth you don't act on is a prettier dashboard. The value has moved to **acting** on memory: a **demand-derived action ontology** (executable actions + decision-support intents, discovered from what merchants ask and do, not hand-authored) governed by typed adapters, with a **merchant-set autonomy slider** (recommend → approve→execute → earned autonomy per action class). Memory is now the *substrate*; the destination is Jefe operating as the eCommerce manager. See the North Star in `AGENTS.md`; a dedicated ontology + autonomy-as-policy direction doc is the recommended next design artefact. **(2026-07-31: the Kinetic layer's FIRST action — dead-stock clearance — is now **LIVE in production** (`CLEARANCE_EXECUTE_ENABLED=true`); the full belief → decision → action → outcome → belief loop is active, inert only until a connected store has costed dead stock to clear. See gaps #1 / #5.)**
...
- **Learn loop (partial)** — **recommendation-engagement** capture; **gap-driven open questions** that raise *and retract* themselves as data fills (e.g. "add your product costs"). **(2026-07-30: now also clearance **outcome measurement** → `business.clearance_effectiveness.all_time`, plus `business.action_decline_signal.all_time` from declined suggestions, refreshed by a daily worker job — both live but inert until a store's first real clearance is scored.)**
...
1. **The Kinetic layer — first action BUILT (dark); broader ontology still open.** *(updated 2026-07-30)* Dead-stock clearance is built end-to-end behind `CLEARANCE_EXECUTE_ENABLED`: a typed **reversible** adapter (idempotent, preview, blast-radius cap, floored-at-cost), the `action_executions` ledger, an LLM **action-intent → `proposeActionFromIntent`** binding (the LLM picks the verb from a capability registry; the primitive computes safe floored params), the **3-mode autonomy dial** (recommend / approve_execute / autonomous, per merchant × action-type), the Approve / Decline+reason / Edit-% surface, and the Observe→Learn outcome loop. Flag-off dry-run passed and **go-live is DONE** (2026-07-31: `CLEARANCE_EXECUTE_ENABLED=true` in production); the path is live, inert until a store has costed dead stock. STILL OPEN: the *demand-derived action ontology* (action types beyond clearance, discovered from what merchants ask/do) + autonomy-as-policy governance. The single-action foundation is proven; **breadth** is the remaining Kinetic work.
...
5. **The Learn loop — outcome measurement BUILT for clearance; generalization open.** *(updated 2026-07-30)* Clearance outcomes are now measured back into memory (`clearance_effectiveness` + `action_decline_signal` beliefs, refreshed by a daily worker job), live but inert until a store's first real clearance is scored. STILL OPEN: feeding these outcomes into belief **confidence** + the **earned-autonomy ladder** (auto-graduate an action class once it consistently works), and generalizing the outcome-measurement pattern beyond clearance to each new action type.
