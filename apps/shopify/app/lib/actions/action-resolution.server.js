...
export async function getActiveSuggestedAction(prisma, input) {
  const row = await prisma.actionExecution.findFirst({
    ...
    select: {
      ...
      preview: true,
      // The raise payload. buildEligibilityRecord has been writing modeReason /
      // policyViolations / degradedFromAutonomous into this column — "here's what I couldn't
      // do, and what you'd need to change" — and nothing ever selected it, so Jefe worked out
      // why it could not act and then dropped it one step before the merchant.
      eligibility: true,
    },
  });
  if (!row) return null;
