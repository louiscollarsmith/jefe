...
test("NON_EXECUTABLE with a scope_missing family and zero attemptable ops is EXECUTION_SEMANTICS_MISSING", () => {
  const f = family({
    capabilityState: "scope_missing",
    executionSummary: { executable: 0, executableWithConfirmation: 0, unsupportedSemantics: 12, prohibited: 0 },
  });
  assert.equal(
    classifyDispositionDetail({ candidateStatus: "NON_EXECUTABLE", reason: "no safe mutation exists", family: f }),
    CANDIDATE_DISPOSITION_DETAIL.executionSemanticsMissing,
  );
});

test("NON_EXECUTABLE with attemptable ops present but scope not satisfied is SCOPE_NOT_GRANTED", () => {
  const f = family({
    capabilityState: "scope_missing",
    executionSummary: { executable: 0, executableWithConfirmation: 1, unsupportedSemantics: 36, prohibited: 0 },
  });
  assert.equal(
...
test("NON_EXECUTABLE with an all-prohibited family is SAFETY_PROHIBITED", () => {
  const f = family({
    capabilityState: "scope_missing",
    executionSummary: { executable: 0, executableWithConfirmation: 0, unsupportedSemantics: 0, prohibited: 2 },
  });
  assert.equal(
    classifyDispositionDetail({ candidateStatus: "NON_EXECUTABLE", reason: "irreversible financial mutation", family: f }),
    CANDIDATE_DISPOSITION_DETAIL.safetyProhibited,
  );
});
