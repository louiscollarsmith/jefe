...
- Every subsystem is observable by default: it logs through the structured logger (credential masking only since 2026-08-13 — see the logging rule below), lets failures surface to the central error hooks, and exposes a health/self-check for any new service or external dependency.
...
- Log all server-side events through the structured logger (`apps/shopify/app/lib/observability/logger.server.js`), never bare `console.*`. Log identifiers and metadata, never request/response payloads, secrets or customer PII. ⛔ **There is no longer a safety net underneath that rule.** PII scrubbing was removed across every surface on 2026-08-13 (founder's call): `redact()` masks **credentials only**, so a customer email or phone number passed into log context now reaches stdout, Sentry and the ops activity log verbatim. Discipline at the call site is the whole protection. (Do not re-add scrubbing because it looks missing — its absence is deliberate. See `apps/shopify/CHANGELOG.md`, 2026-08-13.) Let thrown errors surface to the central hooks (`handleError` in `entry.server.tsx`, route error boundaries) rather than swallowing them, and add a health signal for any new external dependency. See `apps/shopify/docs/observability.md`.
