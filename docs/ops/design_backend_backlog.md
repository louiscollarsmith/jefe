...
4. **"While you slept" executed-action feed (with £ impact).** Design lists store actions Jefe took ("moved £600", "rewrote 18 titles") with money impact. **Jefe takes no external store actions and stores no action log** (by design — advisory only, per CLAUDE.md), and there's no impact field. → Requires (a) the action-execution capability itself and (b) an Action/outcome model. Until then: reframe as **memory/analysis activity** (refreshed memory, learned N facts from `MerchantMemoryBeliefHistory`, regenerated insights/goals/plan) with no £ impact — or omit.
