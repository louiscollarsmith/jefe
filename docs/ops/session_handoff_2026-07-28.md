...
## Running (background agent) — will notify the next window
- **Email MVP** — `app/lib/email/*` (Resend adapter, `ENABLE_EMAIL` off = no real send), welcome template, install-welcome trigger (in shopify.server.js / tenant.server.js), `resend` npm pkg, `.env.example`. TEST-MODE only; must never send real email.
