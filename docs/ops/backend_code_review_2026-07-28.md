...
> **Status update — 2026‑07‑28 (evening), added by the Merchant Memory doc-refresh pass.** Much of the "go‑live hardening" below has since shipped; treat `apps/shopify/CHANGELOG.md` as source of truth. **Closed:** Tier 1 #1 GDPR/compliance webhooks (real redact/export/teardown in `compliance.server.js`, handled *before* the ledger write; plaintext customer email no longer stored); Tier 1 #2 line‑item total bug (+ test); Tier 1 #3 numeric grounding (now grounds against belief *values* only, excludes id/confidence, goals included); Tier 2 #5 multi‑write atomicity (belief + history + evidence in one transaction); Tier 3 #9 Insights/Goals input bounding (top‑40 prioritised beliefs). **Partially closed:** Tier 2 #6 belief staleness — deterministic obsolete‑reconciliation shipped, and confirming a time‑varying observation now stays re‑derivable at system precedence (no more freeze‑at‑1.0). **Still open:** Tier 2 #4 job‑row lifecycle races; Tier 3 #7 secret posture (Shopify offline token still plaintext); Tier 3 #8 visible‑memory provenance/confidence *display*; Tier 3 #10 schema/enum + hardcoded‑currency hygiene (now higher‑stakes — the operator demand signal is heavily multi‑market/multi‑currency); remaining Tier 3 #11 test gaps. Net: the "wrong‑number‑as‑fact" theme this review led with is largely addressed; the frontier has moved from *hardening* to *memory fidelity* (see `apps/shopify/docs/merchant-memory-state.md`).
