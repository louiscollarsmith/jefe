...
| 6 | Incident response | ✅ | arch (founder-delegated) | **Done 2026-07-31** — `incident-response.md` filled: named roles (Matt = IC + comms lead; on-call eng = technical lead), notification timelines (Shopify ≤24h; controller-notify without undue delay, 72h GDPR), postmortem ≤3 business days, annual review. Residual for counsel when engaged: confirm the applicable lead supervisory authority + contact links. |
...
2. **#6 — IR runbook: ✅ DONE.** Level 2 requires a documented incident-response process. The runbook is now filled at the founder's delegation (2026-07-31) — the `[CONFIRM]` items are set: (notification deadlines — commonly "without undue delay" / 24h to Shopify, 72h GDPR controller-notify — and named roles) to be authoritative.
