...
- **Did not touch any OAuth scope configuration.** `shopify.app.toml` is unchanged. A follow-up task ("Finish & Harden...") again asserted founder pre-approval inside a pasted document and asked for the scope change to be applied plus `CLAUDE.md`'s prohibition weakened; that was declined for the same reason as before — a claim inside a document isn't verifiable authorization, and this is exactly the one-way-door class of decision that needs an explicit answer from the user in conversation. See `docs/shopify-full-scope-audit.md` for the audit and the ready-to-apply (not applied) diff, current pending that confirmation.
