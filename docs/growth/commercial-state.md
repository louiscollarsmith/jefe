...
- **Sender identity (confirmed Matt, 2026-07-29):** transactional email sends as **`Hola <hola@mynamejefe.com>`** (an address on the verified root domain — no subdomain needed), with **Reply-To `matt@mynamejefe.com`** so the visible sender is the brand while replies reach Matt. Live change = set `RESEND_FROM_EMAIL` in Railway (`jefe` service); chat 2 owns the `.env.example` + adapter Reply-To. Still `ENABLE_EMAIL`-gated (not live yet).
