...
## 1. Positioning (accurate — three modes, autonomy from day 1)
...
- **Three modes by design (the capability).** Jefe operates in three merchant-controlled modes, chosen **per action type**: (a) **recommend** — Jefe surfaces the move with its evidence, the merchant acts; (b) **approve-execute** — Jefe proposes, the merchant approves, Jefe executes; (c) **autonomous** — Jefe acts within the merchant's policy and reports back. All three run through the same guardrails: typed adapters, idempotency, previews, approval gates, blast-radius caps, reversibility. The merchant is always the **principal** — sets goals + per-action-type mode, and can veto or reverse anything. (Canonical: `AGENTS.md` → North Star; direction confirmed by Matt via chat 7, 2026-07-30.)
- **Launch reality — autonomy from day 1 (Matt-confirmed via chat 7, 2026-07-30).** Jefe **executes on the merchant's behalf, live at launch** — three modes per action type (recommend · approve-execute · autonomous). chat 7 is shipping the live Shopify write client and **enabling execution before submit**, so "Jefe executes" is **true at review time**. **Honesty gate:** the "executes" claim in legal + the listing is valid **only if execution is actually LIVE at submit** (chat 7 owns landing + confirming it); if it isn't, fall back to recommends-now wording — never claim to a reviewer an action they can't see. Confirmed canonical line: *"Jefe acts on your behalf — you approve each action, or let it run autonomously, per action type."*
...
- **2026-07-31 — 🚀 App Store submission SENT.** Jefe submitted to the Shopify App Store → in review (~2 weeks, incl. the Level-2 data-protection review). Landed: full listing (name *Jefe - Your AI Store Manager*, Workflow-automation category, autonomy-from-day-1 three-mode copy, 7 scopes, free/early-access plan, icon + screenshots + demo video); all three legal pages live (`/privacy` · `/terms` · `/dpa`); execution live in prod; ops-panel re-gated + PII-safe access-logging. Post-submit, during the review window: finalize the Level-2 substance (IR runbook = *the Directors* + notify-without-undue-delay; enable the retention purge on Matt's window), build the `/early-access` unlisted-onboarding page, confirm the Neon-vs-Railway sub-processor DB. Records: [launch-checklist.md](launch-checklist.md) · [listing-submission-sheet.md](listing-submission-sheet.md).
- **2026-07-30 (pm)** — **Autonomy framing settled: THREE modes, autonomy FROM DAY 1** (§1 rewritten). Matt confirmed (relayed by chat 7) that Jefe **executes on the merchant's behalf, live at launch** — three modes per action type: **recommend · approve-execute · autonomous** (Matt explicitly rejected propose-first/flagged-off). chat 7 is shipping the live Shopify write client + enabling execution **before submit**, so the "executes" claim is true at review time (honesty gate: must be live at submit, else fall back to recommends-now). Direction-accurate line (exact wording still Matt's to confirm, chat 7 relays): *"Jefe acts on your behalf — you approve each action, or let it run autonomously, per action type."* Applies across Terms/Privacy/listing/commercial-state. Scope: **keep all `write_*`** (execution uses them; trimming then re-requesting forces merchant re-consent). Category: Workflow automation (execution live/demonstrable at launch). (Framing iterated this session — install-claim → propose-first interim → three-mode/autonomy-from-day-1 final — as the code caught up to the intent.)
