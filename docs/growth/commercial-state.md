...
- **2026-07-30 (pm)** — **Autonomy framing settled: two-mode, autonomy FROM DAY 1** (§1 rewritten). Matt confirmed (relayed by chat 7) that Jefe **executes on the merchant's behalf, live at launch** — two modes per action type (approve-each **or** autonomous), not propose-first/advisory. chat 7 is shipping the live Shopify write client + enabling execution **before submit**, so the "executes" claim is true at review time (honesty gate: must be live at submit, else fall back to recommends-now). Canonical line: *"Jefe acts on your behalf — you approve each action, or let it run autonomously, per action type."* Applies across Terms/Privacy/listing/commercial-state. Scope: **keep `write_products`** (clearance executes); the other four write_* noted as trim-candidates (unused). (This session iterated the framing — install-autonomy claim → propose-first interim → autonomy-from-day-1 final — as the code caught up to the intent.)
