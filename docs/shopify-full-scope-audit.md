...
`read_third_party_fulfillment_orders` / `write_third_party_fulfillment_orders` — **APPLICABLE_TO_JEFE, but only relevant for merchants using third-party fulfillment**; low priority to request until there's demonstrated demand.

`read_marketplace_fulfillment_orders` — **APPLICABLE_TO_JEFE in principle, but excluded** (2026-08-25, founder instruction, in conversation): removed from `shopify.app.toml`/`shopify.app.staging.toml`/`.env.example` after review found it was declared but never actually requested by this audit's own §3.1 reasoning above.
