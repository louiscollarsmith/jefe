...
6. **Observability by default** (`lib/observability`): structured logger with always-on **credential** masking, a central error hook → Sentry + the activity log, correlation ids across jobs. Health: `/health` liveness, `/ready` readiness. ⛔ PII scrubbing was removed across every surface on 2026-08-13 (founder's call) — customer emails, phone numbers and names now flow into logs, Sentry and Merchant Memory unmasked. Keeping personal data out is a call-site discipline, not something the platform enforces.
